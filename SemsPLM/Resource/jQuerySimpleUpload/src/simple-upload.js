  ...
  validate(files) {
    let passed = [], rejected = [];
      ...
      //} else if (this.options.allowedFileName && !file.name.match(this.options.allowedFileName)) {
          //modify by ihjung
      } else if (this.options.allowedFileName.length > 0 && $.inArray(file.name, this.options.allowedFileName) < 0) {
        file.reason = 'name';
        rejected.push(file);
      //} else if (this.options.allowedMimeType && !file.type.match(this.options.allowedMimeType)) {
      } else if (this.options.allowedMimeType.length > 0 && $.inArray(file.type, this.options.allowedMimeType) < 0) {
        file.reason = 'type';
        rejected.push(file);
      } else {
        passed.push(file);
