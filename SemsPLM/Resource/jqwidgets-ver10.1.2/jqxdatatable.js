...
( function ( $ ) {
    ...
    $.extend( $.jqx._jqxDataTable.prototype, {
        ...
        _updateScrollbars: function ( widgetHeight ) {
            ...
            if ( tableWidth > hostWidth && !autoWidth ) {
                ...
                if ( !autoHeight ) {
                    ...
                    var tmpVal = this.vScrollBar.jqxScrollBar('value');
                    if ((tableHeight - hostHeight) - tmpVal < vOffset) {
                        tmpVal = tmpVal + vOffset;
                    } else {
                        tmpVal = tmpVal + 1;
                    }
                    this.vScrollBar.jqxScrollBar('setPosition', tmpVal);
                    //ended modify by ihjung
                }
            }
            else {
                this.hScrollBar[ 0 ].style.visibility = "hidden";
        ...
        beginroweditbykey: function ( key, editColumn ) {
            ...
            if (isCellEdit && editColumn &&
                (!editColumn.editable ||
                    (this.rowinfo[key].row.diseditable != undefined && this.rowinfo[key].row.diseditable.length > 0 && this.rowinfo[key].row.diseditable.indexOf(editColumn.datafield) > -1))) {
            //Endted Modify by ihjung
                this.editKey = null;
                return false;
            }
