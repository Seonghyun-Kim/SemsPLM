                ...
                if (this.allowDrag && this.host.jqxDragDrop) {
                    ...
                    if (elements.length > 0) {
                        elements.jqxDragDrop({
                            ...
                            initFeedback: function (feedback) {
                                //modify by ihjung
                                if (feedback.hasClass('jqx-tree-item-hover')) {
                                    var tmpHtml = '';
                                    var checkElements = me.host.find('.jqx-checkbox-check-checked');
                                    for (var index = 0; index < checkElements.length; index++) {
                                        if (index == 0) {
                                            tmpHtml = checkElements.eq(index).parent().parent().parent().next().html();
                                        } else {
                                            tmpHtml += '<br />' + checkElements.eq(index).parent().parent().parent().next().html();
                                        }
                                    }
                                    feedback.html(tmpHtml);
                                }
                                var title = $('<span style="white-space: nowrap;" class="' + me.toThemeProperty('jqx-fill-state-normal') + '">' + feedback.text() + '</span>');
                                $(document.body).append(title);
                                var width = title.width();
